...
1. **Organization Management**: `getOrganizationList`, `createOrganization`, `updateOrganization`, `getOrganizationById`, `deleteOrganization`
...
8. **User Management & RBAC**: `getUserList`, `updateUserPermissions`, `getRolePreset`, and `checkPermission`. Manages roles (`System Admin`, `Organization Admin`, `Organization EEC Officer`, etc.) and granular module-level permissions.
23. **API Security Guards**: `_requireWrite` (checks module-specific write permissions) and `_requireOrganizationMatch` (validates Organization data scoping for restricted roles). These ensure that any data manipulation is authorized at the function-entry point.
...
1. **Authentication & RBAC Lifecycle**:
   ...
   - **New Users**: Redirected to a registration form (`register.html`) to select their Organization.
...
2. **Context Selection Workflow (Organization Selector)**:
   - Admins can swap their Organization context via the `#organization-selector`.
   - Organization-restricted roles have this selector locked to their assigned Organization via `ui.js`.
3. **Asset & Data Management Workflow**:
   ...
   - The controller queries `api.js` to get the assets associated with `currentOrganizationId` and seamlessly generates DOM table nodes for the `#app-content` area.
...
5. **Automated Compliance Reporting Workflow**:
   - The Reporting mechanism pools together entities scoped to the Organization: Assets, monthly Electricity/Fuel Consumptions, planned RIOs, and PPAs. 
